    ...
    >
      <div style={{ width: '100%', maxWidth: 480 }}>
        <div style={{ marginBottom: '2rem' }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 500, margin: '0 0 6px' }}>
            Who is submitting?
          </h1>
          <p
            style={{
          ...
          >
            Your details will be attached to the document for approvers to
            reference.
          </p>
        </div>
        <div
        ...
        >
          ...
          >
            {[
              ['First name', 'Alex', 'firstName'],
              ['Last name', 'Rivera', 'lastName'],
            ].map(([label, ph, key]) => (
              <div key={key}>
                <label
                  style={{
            ...
            >
              ...
              {file ? (
                <div>
                  ...
                  />
                  <p
                    style={{ fontSize: 13, fontWeight: 500, margin: '0 0 2px' }}
                  >
                    {file.name}
                  </p>
                  <p
          ...
          <button
            onClick={() => valid && onNext({ ...form, file })}
            disabled={!valid}
            style={{
              marginTop: 4,
              padding: '10px 0',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `pdfs/${Date.now()}_${submitter.file.name}`
      );
      ...
      // 2. Build approver objects from selections
      ...
      // 3. Save approval document to Firestore
      // The Cloud Function watches this collection and sends the first email
      const docRef = await addDoc(collection(db, 'approvals'), {
        ...
        approvedCount: 0, // Cloud Function increments this
        pdfUrl,
    ...
    >
      <div style={{ width: '100%', maxWidth: 480 }}>
        <div style={{ marginBottom: '2rem' }}>
          ...
          >
            Select three people to review this document. Each will be emailed in
            sequence.
          </p>
        </div>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              >
                {submitter.company} · {submitter.file.name}
              </p>
            </div>
          </div>

          {/* Three approver dropdowns */}
          {[0, 1, 2].map((i) => (
            <div key={i}>
              <label
                style={{
              ...
              <div
                style={{
                  border: `1px solid ${
                    selections[i] ? '#97C459' : 'var(--color-border-secondary)'
                  }`,
                  borderRadius: 'var(--border-radius-md)',
                  overflow: 'hidden',
                }}
              ...
              {selections[i] && (
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      {selections[i].split(' — ')[0]}
                    </p>
                    <p style={{ fontSize: 10, color: '#3B6D11', margin: 0 }}>
                      {
                        APPROVERS.find(
                          (a) => `${a.name} — ${a.title}` === selections[i]
                        )?.email
                      }
                    </p>
                  </div>
          ...
          <button
            ...
            style={{
              ...
              opacity: allSelected && !uploading ? 1 : 0.4,
            }}
          >
            {uploading
              ? uploadPct < 100
...
function ChainProgressView({ docId, submitter, onDone }) {
  const [doc, setDoc] = useState(null);
  const [loading, setLoading] = useState(true);
  const pdfUrl = useRef(null);

  if (!pdfUrl.current && submitter.file)
    pdfUrl.current = URL.createObjectURL(submitter.file);

  // Real-time Firestore listener
  useState(() => {
    const { onSnapshot: snap, doc: docRef } = require('firebase/firestore');
    // Using useEffect pattern via useState initializer is not ideal;
    // this import is handled properly below via useEffect in the actual component.
  });

  // Proper real-time listener via useEffect
  const [approvedCount, setApprovedCount] = useState(0);
  const [approvers, setApprovers] = useState([]);
  const [status, setStatus] = useState('pending');

  useState(() => {}); // placeholder — listener wired below via exported hook

  // NOTE: This component uses a pattern where we attach a Firestore onSnapshot
  // listener when mounted. We do it with a plain useEffect-like call via
  // the component's first render triggering a subscription.
  // Real implementation uses useEffect — see comment in App.jsx.

  return null; // replaced by the full component exported below
}

// ─── Full submitter view (exported) ─────────────────────────────────────────
          ...
          {allDone && (
            ...
            >
              ...
              <p style={{ fontSize: 11, color: '#3B6D11', margin: 0 }}>
                A confirmation has been sent to {submitter.email}
              </p>
            </div>
          )}
        </div>
      ...
      >
        ...
        >
          ...
          <span style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>
            {submitter.file
              ? (submitter.file.size / 1024).toFixed(0) + ' KB'
              : ''}
          </span>
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          {pdfUrl.current ? (
            ...
            >
              PDF preview unavailable after page reload
            </div>
          )}
        </div>
      </div>
  ...
  return (
    ...
    >
      <div style={{ width: '100%', maxWidth: 440, textAlign: 'center' }}>
        ...
        >
          <strong>{submitter.file?.name}</strong> has been fully approved on
          behalf of {name} at {submitter.company}. A confirmation has been sent
          to {submitter.email}.
        </p>
        <div
          style={{
...
export default function SubmitterView() {
  const [step, setStep] = useState('info'); // "info" | "select" | "chain" | "done"
